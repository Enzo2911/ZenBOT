...
module.exports = {
    ...
    description: "Permet de générer un compte kfc si vous est Platinium ou Ultimate",
    ...
    run: async (Alexa, message, args, prefix, log) => {
        db.query(`SELECT platinium as plat, ultimate as ulti, nbgenplat, nbgenulti FROM registre WHERE id = ${message.author.id}`, async (error, results) => {
            ...
            if ((results[0].plat === 1) || (results[0].ulti === 1)) {
                db.query(`SELECT id, user, pass, capture FROM genkfc`, async (error, results2) => {
                    if (error) throw error;
                    if (results2[0] !== undefined) {
                        if (results[0].plat === 1) {
                            ...
                            message.author.send(`Voici le Compte kfc : ${results2[0].user}:${results2[0].pass} // Capture :  ${results[0].capture}`)
                            db.query(`DELETE FROM genkfc WHERE id = ${results2[0].id}`, async (error) => {
                                if (error) throw error;
                            })
                        ...
                        message.reply("Désolé il n'y a plus de compte kfc");
                    }
                })
            } else {
