...
module.exports = {
    ...
    description: "Permet de générer un Compte Del Arte si vous est Platinium ou Ultimate ou AutoHit",
    ...
    run: async (Alexa, message, args, prefix, log) => {
        db.query(`SELECT autohit, platinium as plat, ultimate as ulti, nbgenplat, nbgenulti, nbgenautohit FROM registre WHERE id = ${message.author.id}`, async (error, results) => {
            ...
            if ((results[0].plat === 1) || (results[0].ulti === 1) || (results[0].autohit === 1)) {
                db.query(`SELECT id, user, pass, capture FROM gendelarte`, async (error, results2) => {
                    if (error) throw error;
                    if (results2[0] !== undefined) {
                        if (results[0].plat === 1) {
                            ...
                            message.author.send(`Voici le Compte Del Arte : ${results2[0].user}:${results2[0].pass} // Capture :  ${results[0].capture}`)
