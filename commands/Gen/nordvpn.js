...
module.exports = {
    ...
    description: "Permet de généré un Compte NordVPN si vous êtes Platinium ou Ultimate ou AutoHit",
        ...
        if (message.channel.id !== config.channel) {
            ...
            db.query(`SELECT autohit, platinium as plat, ultimate as ulti, nbgenplat, nbgenulti, nbgenautohit FROM registre WHERE id = ${message.author.id}`, async (error, results) => {
                ...
                if ((results[0].plat === 1) || (results[0].ulti === 1) || (results[0].autohit === 1)) {
                    db.query(`SELECT id, user, pass, capture as cap FROM gennordvpn`, async (error, results2) => {
                        ...
                        if (results2[0] !== undefined) {
                            if (results[0].plat === 1) {
                                ...
                                message.author.send(`Voici le Compte NordVPN : ${results2[0].user}:${results2[0].pass} // Capture :  ${results2[0].cap}`).catch(() => message.reply("Active tes DM plz"));
                                console.log("")
                                ...
                                db.query(`DELETE FROM gennordvpn WHERE id = ${results2[0].id}`, async (error) => {
                                    if (error) throw error;
                                })
                            ...
                            message.reply("Désolé il n'y a plus de Compte NordVPN");
                        }
                    })
                } else {
