...
module.exports = {
    ...
    description: "Permet de généré un Compte King365 si vous êtes Platinium ou Ultimate ou AutoHit",
                    ...
                    db.query(`SELECT id, user, pass, capture as cap FROM genking365`, async (error, results2) => {
                        ...
                        if (results2[0] !== undefined) {
                            if (results[0].plat === 1) {
                                ...
                                message.author.send(`Voici le Compte King365 : ${results2[0].user}:${results2[0].pass} // Capture :  ${results2[0].cap}`).catch(() => message.reply("Active tes DM plz"));
                                console.log("")
                                ...
                                db.query(`DELETE FROM genking365 WHERE id = ${results2[0].id}`, async (error) => {
                                    if (error) throw error;
                                })
                            ...
                            message.reply("Désolé il n'y a plus de Compte King365");
                        }
                    })
                } else {
